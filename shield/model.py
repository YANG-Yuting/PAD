...
class RobertaClassifierDARTS(nn.Module):
    def __init__(self,
                 model_type,
                 output_dim=2,
                 freeze_roberta=True,
        ...
        super(RobertaClassifierDARTS, self).__init__()
        self.name = model_type
        self.scaler = scaler
        self.gumbel = gumbel
        self.embedding_matrix = []
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_type)
        self.roberta_layer = AutoModel.from_pretrained(model_type)
        ...
        self.emb1 = None
        self.temperature = temperature
        self.embedding = self.roberta_layer.get_input_embeddings()
        self.dropout = nn.Dropout(0.5)
        self.L = 3
        self.sample_wise_training = True
        self.darts = darts
        ...
        for module in self.roberta_layer.modules():
            if isinstance(module, nn.Embedding):
                if module.weight.size()[0] >= 30000:
                    self.embedding_weight = module.weight.data.clone()
                    self.embedding_dim = self.embedding_weight.size()[1]
        ...
        if self.ensemble:
            if self.darts:
                ...
                self.heads = nn.ModuleList(
                    [nn.ModuleList([nn.Sequential(nn.Linear(self.feature_dim, self.output_dim))]) for i in
                     range(self.N)])
    ...
    def forward_from_embedding(self, emb, attn_masks):
        cont_reps = self.roberta_layer(inputs_embeds=emb, attention_mask=attn_masks)
        pred = cont_reps.last_hidden_state[:, 0]
        ...
        if self.ensemble > 0:
            ...
            self.random_key = self.select_head_gumbel(pred, self.pred_heads)
            pred = torch.cat([(self.pred_heads[i] * self.random_key[:, i].unsqueeze(1)).unsqueeze(1) for i in
