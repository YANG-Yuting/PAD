...
def log_det(y_true, y_pred, num_model=args.num_models):  # y_true为标签值
    ...
    #     all_log_det = torch.logdet(matrix + det_offset * torch.unsqueeze(torch.eye(num_model).cuda(args.rank), 0))
    #y_pred = torch.reshape(y_pred, [-1, num_model, args.num_labels])
    y_pred = torch.reshape(y_pred, [-1, num_model, args.max_seq_length*768])
    y_pred = y_pred / torch.norm(y_pred, dim=2, keepdim=True)
    matrix = torch.matmul(y_pred, torch.transpose(y_pred, 2, 1))
    all_log_det = torch.logdet(matrix + det_offset * torch.unsqueeze(torch.eye(num_model).cuda(args.rank), 0))
    return all_log_det


def Loss_withEE_DPP(y_true, y_pred,aux_out, num_model=args.num_models): # y_pred [batch_size*num_models, num_classes]
    ...
    if args.lamda == 0:
        ...
        log_dets = log_det(y_true, aux_out, num_model)
        ...
        for idx, (*train_x, train_y) in enumerate(dataloader_train):
            ...
            if torch.cuda.device_count() > 1:
                ...
                for i in range(args.num_models):
                    ...
                    _, lg = model.module.bert(input_ids, token_type_ids=segment_ids, attention_mask=input_mask, model_id=i)
                    ...
                    aux_outs.append(torch.reshape(model.module.bert.bert.encoder.layer[0].attention.self.params_aux,[train_y.shape[0],-1]))
                    # 将bert参数重置为初始值
                    # print('-----------')
                    # for name, param in model.bert.named_parameters():
                ...
                aux_outs=torch.cat(aux_outs,dim=0)
            else:
                logits = []
                true_labels = []
                aux_outs=[]
            ...
            loss, loss_ce, loss_e, loss_d = Loss_withEE_DPP(true_labels, logits,aux_outs)
...
def main(args):
    ...
    # if args.target_model == 'bert':
    #print(model.module.bert.bert.named_parameters())
    # for b_name, block in model.module.bert.bert.named_parameters():
    #     print(b_name)
    #     print(block)
    # # for p in model.parameters():
    # #     print(p)
    # exit(0)
    #T_parameters = filter(lambda p: 'aux' in p[0], model.module.bert.bert.named_parameters())
    #     if torch.cuda.device_count() > 1:
    #         if args.task == 'snli':
    ...
    if is_main_process():
        ...
        #para = sum(p[1].numel() for p in T_parameters)
    ...
    # P_list=[]
    # for p,p1 in T_parameters:
    #     P_list.append(p1)
    # print([p[1] for p in T_parameters])
    # optimizer = optim.Adam(P_list, lr=0.001)
    epoch = 200  # 10
    best_test = 0
    # params_aux_s = {}
    for e in range(epoch):
        if e==20:
            optimizer = optim.Adam(filter(need_grad, model.parameters()), lr=args.lr*0.1)
        if torch.cuda.device_count() > 1:
