...
class TransformersClassifier(Classifier):
    ...
    def predict(self, sen_list, labels=None):
        ...
        # print(self.__tokenizer.cls_token_id, self.__tokenizer.sep_token_id, self.__tokenizer.pad_token_id)
        ...
        for i in range( (len(sen_list) + self.batch_size - 1) // self.batch_size):
            ...
            if i == 0:
                ...
                if args.target_model == 'bart':
                    all_hidden_states = torch.cat((all_hidden_states, outputs.decoder_hidden_states[-1].detach().cpu()), dim=0)
                else:
    ...
    def get_hidden_states(seelf, input_, labels=None):
        """
